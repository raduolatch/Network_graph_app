...
# Bug-fix: avg_path & diameter dihitung via safe wrapper (tidak freeze graph besar)
...
with tab2:
    ...
    if len(node_list) >= 2:
        ...
        if st.button("Cari Rute Terpendek", use_container_width=True):
            # Bug-fix: source == target sekarang ditangani oleh dijkstra_shortest_path
            result = dijkstra_shortest_path(G, source_node, target_node)

            if result["success"]:
                shortest_path = result["path"]
                # Bug-fix: truncate path panjang agar tidak overflow UI
...
with tab3:
    ...
    # Bug-fix: compute_centrality() pakai cache di session_state,
    # tidak dihitung ulang setiap kali tab dibuka
...
with tab4:
    ...
    if selected_node is not None:
        ...
        # Bug-fix: ambil dari cache, tidak hitung ulang O(n³) per klik
        ...
        # Bug-fix: truncate + tampilkan "+N more" via neighbor_chips()
